...
// ========== API الأساسية ==========
app.get('/api', (req, res) => {
    res.json({ name: 'سوريا الإلكترونية', version: '6.0', status: 'running' });
});
...
app.put('/api/regions/:id', (req, res) => {
    try {
        ...
        run('UPDATE regions SET name_ar = ?, population = ?, image_url = ? WHERE id = ?',
            [name_ar || null, population || null, image_url || null, req.params.id]);
        saveNow();
        res.json({ success: true });
    } catch(e) { res.json({ success: false }); }
});
    const { admin_level_id, parent_id } = req.query;
    let query = 'SELECT * FROM regions WHERE is_active = 1';
    const params = [];
...
app.get('/api/regions/:id', (req, res) => {
    ...
    if (!r) return res.status(404).json({ success: false, message: 'غير موجود' });
...
app.get('/api/businesses', (req, res) => {
    ...
    let query = `SELECT b.*, r.name_ar as region_name FROM businesses b LEFT JOIN regions r ON b.region_id = r.id WHERE b.is_active = 1`;
    const params = [];
...
app.put('/api/businesses/:id', (req, res) => {
    try {
        const { name_ar, phone, sub_category_id, region_id, description_ar, is_featured, logo } = req.body;
        run(`UPDATE businesses SET name_ar = ?, phone = ?, sub_category_id = ?, region_id = ?, description_ar = ?, is_featured = ?, logo = COALESCE(?, logo) WHERE id = ?`,
            [name_ar, phone, sub_category_id, region_id, description_ar || '', is_featured || 0, logo || null, req.params.id]);
        saveNow();
    ...
    try {
        ...
        if (!name_ar || !phone || !password || !sub_category_id || !region_id) {
...
app.post('/api/home-icons', (req, res) => {
    ...
    const ok = run('INSERT INTO home_icons (name_ar, icon_emoji, icon_color, link_url, image_url) VALUES (?, ?, ?, ?, ?)',
        [name_ar, icon_emoji || '', icon_color || '#007A3D', link_url || '/', image_url || '']);
    saveNow();
    res.json({ success: ok, message: ok ? 'تمت الإضافة' : 'خطأ' });
});
...
app.put('/api/craftsmen/:id', (req, res) => {
    ...
    run('UPDATE craftsmen SET name_ar = ?, phone = ?, trade = ? WHERE id = ?',
        [name_ar, phone, trade, req.params.id]);
    saveNow();
    res.json({ success: true });
});
app.delete('/api/craftsmen/:id', (req, res) => {
    ...
    const ok = run('INSERT INTO craftsmen (name_ar, phone, phone2, trade, description_ar, regions) VALUES (?, ?, ?, ?, ?, ?)',
        [name_ar, phone, phone2 || '', trade, description_ar || '', regions || '']);
    saveNow();
    res.json({ success: ok, message: ok ? 'تم التسجيل' : 'خطأ' });
});
...
app.put('/api/businesses/:id', (req, res) => {
    ...
    if (logo) {
        run('UPDATE businesses SET name_ar=?, phone=?, sub_category_id=?, region_id=?, logo=? WHERE id=?',
            [name_ar, phone, sub_category_id, region_id, logo, req.params.id]);
    } else {
        run('UPDATE businesses SET name_ar=?, phone=?, sub_category_id=?, region_id=? WHERE id=?',
            [name_ar, phone, sub_category_id, region_id, req.params.id]);
    }
    saveNow();
    res.json({ success: true, message: 'تم التحديث' });
});
...
initDB().then(() => {
    app.listen(PORT, '0.0.0.0', () => {
        ...
        console.log(`🇸🇾 Syria Platform v6.0 LIVE!`);
    });
}).catch(err => { console.error('❌ فشل تحميل قاعدة البيانات:', err.message); process.exit(1); });
